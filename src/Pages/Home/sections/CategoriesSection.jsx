...
import { Sparkles, Puzzle, Palette, Rocket, Brain, Cpu, ToyBrick, Gamepad2 } from 'lucide-react';

// Expanded category list with fun icons
const categoriesData = [
  ...
  { name: 'STEM toys', icon: Cpu },
  { name: 'Board games', icon: Gamepad2 },
  { name: 'Plush friends', icon: ToyBrick },
  { name: 'Puzzles', icon: Brain },
];
...
const CategoriesSection = ({ selectedCategory, onSelectCategory }) => {
  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          ...
          {categoriesData.map(({ name, icon: Icon }) => {
            ...
            return (
              <button
                ...
                className={`group flex shrink-0 items-center gap-3 rounded-2xl border-2 px-5 py-3 font-bold transition-all hover:scale-105 hover:shadow-lg ${
                  isSelected
      ...
      <p className="mt-3 text-xs font-semibold text-text-muted">Drag the categories sideways to explore more.</p>
    </section>
  );
};
